...
{
    class Gauger_ShortSpaceBetweenTwoEmptyCircles : Gauger
    {
        ...
        public Gauger_ShortSpaceBetweenTwoEmptyCircles(MainUI parent, PictureBox pb, MEASURE_TYPE type)
        {
            this.parent = parent;
        ...
        public override void on_paint(PaintEventArgs pe, PictureBox pb, Image source_image = null, bool bDrawToSourceImage = false)
        {
            ...
            try
            {
                ...
                if (true == m_bHasValidGaugeResult)
                {
                    ...
                    Debugger.Log(0, null, string.Format("222222 top_arc_start_angle = {0}, top_arc_sweep_angle = {1}, top_arc_radius = {2}",
                        top_arc_start_angle, top_arc_sweep_angle, top_arc_radius));
                    ...
                    if (true)
                    {
                        ...
                        get_theta(m_gauge_result_pts[3].x, m_gauge_result_pts[3].y, m_gauge_result_pts[0].x, m_gauge_result_pts[0].y, out_doubles);
                        Debugger.Log(0, null, string.Format("222222 out_doubles[0] = {0}", out_doubles[0]));

                        get_theta(m_gauge_result_pts[3].x, m_gauge_result_pts[3].y, m_gauge_result_pts[2].x, m_gauge_result_pts[2].y, out_doubles);
                        Debugger.Log(0, null, string.Format("222222 out_doubles[0] = {0}", out_doubles[0]));

                        get_theta(m_gauge_result_pts[3].x, m_gauge_result_pts[3].y, m_gauge_result_pts[1].x, m_gauge_result_pts[1].y, out_doubles);
                        Debugger.Log(0, null, string.Format("222222 out_doubles[0] = {0}", out_doubles[0]));
                    }
        ...
        {
            ((Gauger_ShortSpaceBetweenTwoEmptyCircles)dest).m_bHasValidGaugeResult = m_bHasValidGaugeResult;

            for (int n = 0; n < m_gauge_result_pts.Length; n++)
                ((Gauger_ShortSpaceBetweenTwoEmptyCircles)dest).m_gauge_result_pts[n] = m_gauge_result_pts[n];

            ((Gauger_ShortSpaceBetweenTwoEmptyCircles)dest).m_gauged_line_space = m_gauged_line_space;
            ((Gauger_ShortSpaceBetweenTwoEmptyCircles)dest).m_gauged_line_width = m_gauged_line_width;
            ((Gauger_ShortSpaceBetweenTwoEmptyCircles)dest).m_gauged_line_width2 = m_gauged_line_width2;
