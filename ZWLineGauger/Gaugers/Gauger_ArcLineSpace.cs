...
{
    class Gauger_ArcLineSpace : Gauger
    {
        ...
        public Gauger_ArcLineSpace(MainUI parent, PictureBox pb, MEASURE_TYPE type)
        {
            this.parent = parent;
        ...
        public override bool gauge(Image img, byte[] pImageBuf = null, int nWidth = 0, int nHeight = 0, int nBytesPerLine = 0, int nAlgorithm = 0, bool bLocateLineInRealTime = false)
        {
            ...
            out_ints[0] = 0;
            find_arc_line_space(pBuf, in_data, in_crds, out_ints, out_crds);
            if (1 == out_ints[0])
            {
                ...
                m_gauged_line_space = GeneralUtils.get_distance(m_gauge_result_pts[2], m_gauge_result_pts[12]);
        ...
        {
            ((Gauger_ArcLineSpace)dest).m_bHasValidGaugeResult = m_bHasValidGaugeResult;

            for (int n = 0; n < m_gauge_result_pts.Length; n++)
                ((Gauger_ArcLineSpace)dest).m_gauge_result_pts[n] = m_gauge_result_pts[n];

            ((Gauger_ArcLineSpace)dest).m_gauged_line_space = m_gauged_line_space;
            ((Gauger_ArcLineSpace)dest).m_gauged_line_width = m_gauged_line_width;
            ((Gauger_ArcLineSpace)dest).m_gauged_line_width2 = m_gauged_line_width2;
