...
namespace ZWLineGauger.Gaugers
{
    public class ImgOperators
    {
        ...
        public bool find_circle_in_image(Image img, double radius, double dbAreaRatio, double dbSimilarity, ref Point2d center)
        {
            int[] in_data = new int[10];
            int[] out_ints = new int[10];
            ...
            in_doubles[1] = dbAreaRatio;
            in_doubles[2] = dbSimilarity;
        ...
        public bool find_circle_in_cam_by_shape_matching(CameraOps cam, object cam_lock, double m_pixels_per_um, double radius, ref Point2d center, 
            double dbAreaRatio = 1, double dbSimilarity = 0.5)
        {
            Image guide_img;
            lock (cam_lock)
            {
            ...
            if (true == find_circle_in_image(guide_img, radius * m_pixels_per_um, dbAreaRatio, dbSimilarity, ref center2))
            {
                center = center2;
                return true;
            }
        ...
        public void thread_auto_focus(object obj)
        {
            ...
            if (true == m_bFastFocus)
            {
                Debugger.Log(0, null, string.Format("222222 执行自动对焦 111"));
                double speed1 = 0.5;
                double speed2 = 0.08;
            ...
            {
                Debugger.Log(0, null, string.Format("222222 执行自动对焦 222"));
                double speed1 = 0.5;
                double speed2 = 0.08;
                double speed3 = 0.05;
                speed2 = 0.08 + (5 - parent.comboBox_Len.SelectedIndex) * 0.02;
            ...
            {
                Debugger.Log(0, null, string.Format("222222 执行自动对焦 333"));
                if (dbProfilePos < (dbLowerLimit + (Math.Abs(dbLowerLimit - dbUpperLimit) / 2)))
                {
                    Debugger.Log(0, null, string.Format("111111 自动对焦 从下到上"));
