    ...
    public class GraphView : PictureBox
    {
        ...
        public void draw_array_rects_overlay(Bitmap bmp, double zoom_ratio, int rect_left, int rect_top, List<List<rotated_array_rect>> list_list_rects)
        {
            ...
            if (true)
            {
                ...
                //Debugger.Log(0, null, string.Format("222222 off_x [{0},{1}], pixels_per_mm = {2}, m_bitmap_1bit.Height = {3}", 
                //    off_x, off_y, pixels_per_mm, m_bitmap_1bit.Height));
                ...
                for (int p = 0; p < 2; p++)
                {
                    for (int n = 0; n < list_list_rects.Count; n++)
                    {
                        for (int m = 0; m < list_list_rects[n].Count; m++)
                        {
                            ...
                            //Debugger.Log(0, null, string.Format("222222 m = {0}: angle = {1}, list_list_rects[n][m].lefttop = [{2:0.000},{3:0.000}]", 
                            //    m, list_list_rects[n][m].odb_angle, list_list_rects[n][m].left, list_list_rects[n][m].top));

                            //if (m > 0)
                            //    continue;

                            if (Math.Abs(list_list_rects[n][m].odb_angle) < 0.001)
                                top = m_bitmap_1bit.Height - (list_list_rects[n][m].top * pixels_per_mm + off_y) - height;
                            else if (Math.Abs(list_list_rects[n][m].odb_angle - 180) < 0.001)
                                top = m_bitmap_1bit.Height - (list_list_rects[n][m].top * pixels_per_mm + off_y) - height;
                            else
                                top = m_bitmap_1bit.Height - (list_list_rects[n][m].top * pixels_per_mm + off_y) - height;
                    ...
                    if (true == m_parent.m_bIsCtrlKeyPressed)                               // 阵列unit选择
                    {
                        ...
                        for (int n = 0; n < GraphView.m_ODB_thumbnail_array_rects.Count; n++)
                        {
                            ...
                            for (int m = 0; m < vec_rects.Count; m++)
                            {
                                ...
                                double height = vec_rects[m].height * pixels_per_mm;

                                if (Math.Abs(vec_rects[m].odb_angle) < 0.001)
                                    top = m_bitmap_1bit.Height - (vec_rects[m].top * pixels_per_mm + off_y) - height;
                                else if (Math.Abs(vec_rects[m].odb_angle - 180) < 0.001)
                                    top = m_bitmap_1bit.Height - (vec_rects[m].top * pixels_per_mm + off_y) - height;
                                else
                                    top = m_bitmap_1bit.Height - (vec_rects[m].top * pixels_per_mm + off_y) - height;
